	...
	<div class="container py-5">
		<nav class="nav nav-pills flex-column flex-sm-row mb-5">
			...
			<a
				...
				>My Translations <span class="badge badge-light">{{ $count }}</span></a
			>
			@if($user->roles == 3)
			<a
				class="flex-sm-fill text-sm-center nav-link"
					...
					<div class="card bg-light mb-3">
						<div class="card-body">
							<div class="row align-items-center mb-3">
								...
								<div class="col-auto">
									<!-- If service == translation + editing -->
									@if(isset($data['service_type']) && $data['service_type'] == 1)
										<button
											data-toggle="modal"
											data-target="#confirm-proceed-modal-editor"
									...
									@else
										<button
											data-toggle="modal"
											data-target="#confirm-proceed-modal-client"
											class="btn btn-lg btn-primary"
											onclick="setAssignFile('{{ $data['file'] }}', '{{ $data[0]['orderID'] }}')"
										>
											Send to client
							...
							<div class="row">
								...
								<div class="col">
									...
									<p>
										<span>
											@if( $data['style'] == 1 )
											{{ trans('frontend.order.style.1')}}
											@elseif( $data['style'] == 2 )
											{{ trans('frontend.order.style.2')}}
											@elseif( $data['style'] == 3 )
											{{ trans('frontend.order.style.3')}}
											@endif
										</span>
									</p>
								</div>
		...
		@else
			...
			<h3 class="text-muted">No translating</h3>
		@endif
	</div>
</main>
...
<div
	class="modal fade"
	id="confirm-proceed-modal-editor"
	tabindex="-1"
	...
	<div class="modal-dialog modal-dialog-centered">
		<div class="modal-content">
			...
			<div class="modal-footer">
				...
				<button type="button" class="btn btn-primary" onclick="sendToEditor()">Proceed</button>
			</div>
		</div>
	</div>
</div>
...
<div
	class="modal fade"
	id="confirm-proceed-modal-client"
	tabindex="-1"
	aria-labelledby="confirm-proceed-modal-label"
	aria-hidden="true"
>
